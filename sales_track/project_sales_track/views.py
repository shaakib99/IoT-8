...
def ajax_return_data_for_statistics(request):
    if request.is_ajax():
        try:
            ...
            i = 0
            while i < next_months:
                next_months_name.append((today + datetime.timedelta(days = 30 * i)).strftime("%b"))
                next_months_number.append((today + datetime.timedelta(days = 30 * i)).strftime('%m'))
                next_months_year.append((today + datetime.timedelta(days = 30 * i)).strftime('%Y'))
                i += 1
            d = {} # dict Holder
